...
class WheelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < players.length; i++) {
      ...
      // Ãngulo inicial de la sección (empezamos desde arriba: -Ï€/2)
...
class _TiebreakerScreenState extends State<TiebreakerScreen> with TickerProviderStateMixin {
  ...
  void _spinBottle() async {
    ...
    // CÃLCULO CORRECTO del ganador basado en la posición de la flecha
    final normalizedAngle = totalAngle % (2 * pi);
    final anglePerSection = (2 * pi) / playerCount;

    // La flecha apunta hacia arriba (-Ï€/2 en coordenadas canvas)
    // Las secciones se dibujan desde -Ï€/2 en sentido horario
    // Sección 0: desde -Ï€/2 hasta -Ï€/2 + anglePerSection
    // Sección 1: desde -Ï€/2 + anglePerSection hasta -Ï€/2 + 2*anglePerSection, etc.
  ...
  String _transformGrammatically(String text) {
    ...
    final transformations = <RegExp, String>{
      // "llegue" â†’ "llegarás" (subjuntivo â†’ futuro informal)
